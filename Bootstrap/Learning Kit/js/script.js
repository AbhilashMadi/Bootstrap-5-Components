...
function iterateCodeExamples() {
  $(".code-example").each(function () {
    ...
    var toggleCode =
      '<button type="button" class="btn btn-link btn-sm code-toggle-button">Toggle code</button><div class="code-toggle"><div class="btn-group btn-group-sm codepen-button"><button type="button" class="btn btn-primary clipboard-button" data-custom-tooltip="Copied to clipboard"><img src="../img/clipboard.png">&nbsp;&nbsp;&nbsp;Copy to clipboard</button></div><pre><code class="html"></code></pre><div class="codepen" data-type="html"></div></div>';
    ...
    var white_space = "☺";
    try {
      white_space = sample_html
        .match(/\n+\s+\S/)[0]
        .slice(0, sample_html.match(/\n+\s+\S/)[0].length - 1);
    } catch (err) {}
...
function codepenPrefill(code, codeExample) {
  code.each(function () {
    ...
    HTML =
      "<!-- This code is dynamically created with the Learning Kit for the course 'Master Bootstrap 5' from Frontend Workshop. The course is available on frontendworkshop.com -->\n\n";
    ...
    if (type == "html") {
      ...
      HTML = HTML.replace(reStart, "http://placehold.it/")
        .replace(reEnd, "")
        .replace(reSuccess, "/198754")
        .replace(reDanger, "/dc3545")
        .replace(reWarning, "/ffc107")
        .replace(reInfo, "/0dcaf0");
    } else if (type == "css") {
      CSS = CSS + el.html();
    } else if (type == "js") {
      JS = JS + el.html();
    ...
    var data = {
      ...
      description:
        "Dynamically created CodePen from the online course 'Master Foundation 5' available on frontendworkshop.com",
      editors: "100",
      html: HTML,
...
function pageNavigation(codeExample) {
  ...
  componentVariationId = componentVariation
    .text()
    .replace(/\s+/g, "-")
    .replace(/\(|\)/g, "_")
    .replace(/[:]/g, "-")
